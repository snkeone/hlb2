...
export function decideTrade(payload) {
    ...
    // ────────────────────────
    // A.allow=true 時の構造運用:
    // - open中: 既存snapshotを固定（再生成しない）
    // - closed中: b1.snapshotRefreshSec ごとに再評価する（0なら毎tick）
    ...
    // ポジション非保有時は設定間隔で構造を最新化
    // depthSR が不安定でも b1 側で bar フォールバック構造を生成できる
    if (positionStatus === 'closed') {
        const refreshMs = resolveB1SnapshotRefreshSetting(tradeConfig).ms;
        const snapshotCreatedAt = Number(currentStructureSnapshot?.createdAt);
        const snapshotAgeMs = Number.isFinite(snapshotCreatedAt)
            ? ((ts - snapshotCreatedAt) < 0 ? Number.POSITIVE_INFINITY : (ts - snapshotCreatedAt))
            : Number.POSITIVE_INFINITY;
        const shouldRefresh = !currentStructureSnapshot || refreshMs === 0 || snapshotAgeMs >= refreshMs;
        if (shouldRefresh) {
    ...
    const bResult = decideTradeB2(payload, aResult, currentStructureSnapshot, srClusterView);
  ...
  if (bResult.side === 'none') {
    return emitDecision({
      ...
      reasonCode: resolveReasonCode(bResult.reason, REASON_CODE.UNKNOWN),
      source: 'B',
        context: {
            aResult,
  ...
  return emitDecision({
    ...
    reasonCode: resolveReasonCode(
      bResult.reason,
      bResult?.side === 'none' ? REASON_CODE.UNKNOWN : REASON_CODE.ENTRY_ALLOWED
    ),
    source: 'B',
        ...
        const tracePayload = {
            ...
            payload: {
                ...
                decision: decision ? {
                    ...
                    reasonCode: resolveReasonCode(
                        decision.reason,
                        decision?.side === 'none' ? REASON_CODE.UNKNOWN : REASON_CODE.ENTRY_ALLOWED
                    ),
