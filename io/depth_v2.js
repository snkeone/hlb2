...
var DEFAULT_DYNAMIC_SCALE = {
    windowWidthRatio: 0.35,
    mergeWidthRatio: 0.02,
    minWidthRatio: 0.01,
    maxWidthRatio: 0.06,
    windowMinUsd: 60,
    windowMaxUsd: 2500,
    mergeMinUsd: MERGE_DISTANCE,
    mergeMaxUsd: 120,
    minWidthFloorUsd: MIN_WIDTH,
    maxWidthFloorUsd: MAX_WIDTH,
    maxWidthCapUsd: 320,
};
...
function resolveDynamicScale(rawChannelWidth) {
    var channelWidth = Number(rawChannelWidth);
    var hasChannel = Number.isFinite(channelWidth) && channelWidth > 0;
    var safeChannel = hasChannel ? channelWidth : 0;
    var cfg = DEFAULT_DYNAMIC_SCALE;
    var windowUsdEff = hasChannel
        ? clampNumber(safeChannel * cfg.windowWidthRatio, cfg.windowMinUsd, cfg.windowMaxUsd)
        : cfg.windowMinUsd;
    var mergeDistanceEff = hasChannel
        ? clampNumber(safeChannel * cfg.mergeWidthRatio, cfg.mergeMinUsd, cfg.mergeMaxUsd)
        : cfg.mergeMinUsd;
    var minWidthEff = hasChannel
        ? clampNumber(safeChannel * cfg.minWidthRatio, cfg.minWidthFloorUsd, cfg.maxWidthCapUsd)
        : cfg.minWidthFloorUsd;
    var maxWidthLowerBound = Math.max(cfg.maxWidthFloorUsd, minWidthEff);
    var maxWidthEff = hasChannel
        ? clampNumber(safeChannel * cfg.maxWidthRatio, maxWidthLowerBound, cfg.maxWidthCapUsd)
        : maxWidthLowerBound;
    return {
        channelWidthUsd: hasChannel ? safeChannel : null,
        windowUsdEff: windowUsdEff,
        mergeDistanceEff: mergeDistanceEff,
        minWidthEff: minWidthEff,
        maxWidthEff: maxWidthEff,
        source: hasChannel ? 'b15m_channel_hint' : 'fallback_defaults',
    };
}
// ============================================================================
// DepthSRAnalyzer クラス
...
var DepthSRAnalyzer = /** @class */ (function () {
    function DepthSRAnalyzer() {
        ...
        this.state = {
            ...
            lastDynamicScale: resolveDynamicScale(null),
        };
        // Section 7: 60秒タイマー管理
     ...
     */
    DepthSRAnalyzer.prototype.onDepthSnapshot = function (snapshot, mid, scaleHint) {
        ...
        if (this.shouldRunAnalysis()) {
            ...
            this.runFrequencyAnalysis(mid, scaleHint);
        }
     ...
     */
    DepthSRAnalyzer.prototype.runFrequencyAnalysis = function (mid, scaleHint) {
        ...
        var dynamicScale = resolveDynamicScale(scaleHint === null || scaleHint === void 0 ? void 0 : scaleHint.channelWidthUsd);
        this.state.lastDynamicScale = dynamicScale;
        var freqHistogram = buildFrequencyHistogram(this.history, mid, dynamicScale);
        ...
        var supportClusters = mergeAdjacentBins(supportBins, dynamicScale.mergeDistanceEff);
        ...
        var resistanceClusters = mergeAdjacentBins(resistanceBins, dynamicScale.mergeDistanceEff);
        // Section 6 処理4: Primary の center/width計算
        if (supportClusters.length > 0) {
            this.state.S1_center = supportClusters[0].center;
            this.state.S1_width = calculateWeightedStdDev(supportClusters[0], dynamicScale);
        }
        ...
        if (resistanceClusters.length > 0) {
            ...
            this.state.R1_width = calculateWeightedStdDev(resistanceClusters[0], dynamicScale);
        }
        ...
        if (supportClusters.length > 1) {
            ...
            this.state.S0_width = calculateWeightedStdDev(supportClusters[1], dynamicScale);
        }
        ...
        if (resistanceClusters.length > 1) {
            ...
            this.state.R2_width = calculateWeightedStdDev(resistanceClusters[1], dynamicScale);
        }
    ...
    DepthSRAnalyzer.prototype.generateDepthSR = function () {
        ...
        if (!actualReady) {
            return {
                ...
                srScale: this.state.lastDynamicScale,
 ...
 */
export function buildFrequencyHistogram(history, mid, dynamicScale) {
    var _a, _b;
    ...
    for (var _g = 0, history_2 = history; _g < history_2.length; _g++) {
        ...
        for (var _h = 0, _j = snapshot.bids; _h < _j.length; _h++) {
            ...
            if (bid.price <= mid && bid.size >= threshold) {
                if (dynamicScale && Number.isFinite(dynamicScale.windowUsdEff) && Math.abs(bid.price - mid) > dynamicScale.windowUsdEff) {
                    continue;
                }
        ...
        for (var _k = 0, _l = snapshot.asks; _k < _l.length; _k++) {
            ...
            if (ask.price > mid && ask.size >= threshold) {
                if (dynamicScale && Number.isFinite(dynamicScale.windowUsdEff) && Math.abs(ask.price - mid) > dynamicScale.windowUsdEff) {
                    continue;
                }
 ...
 */
export function calculateWeightedStdDev(cluster, dynamicScale) {
    ...
    var minWidth = Number.isFinite(dynamicScale === null || dynamicScale === void 0 ? void 0 : dynamicScale.minWidthEff) ? dynamicScale.minWidthEff : MIN_WIDTH;
    var maxWidth = Number.isFinite(dynamicScale === null || dynamicScale === void 0 ? void 0 : dynamicScale.maxWidthEff) ? dynamicScale.maxWidthEff : MAX_WIDTH;
