...
export async function fetchBar15mBackfill({
  coin = 'BTC',
  neededBars = 26,
  timeoutMs = 6000
} = {}) {
  ...
  const barsToRequest = Math.max(neededBars + 6, Number(process.env.BAR15M_BACKFILL_BARS || 128));
  const now = Date.now();
  const startTime = now - (barsToRequest * 15 * 60 * 1000);
...
export function nextBar15mBackfillDelayMs({ attempt = 1, retryAfterMs = null, success = false, stillInsufficient = false } = {}) {
