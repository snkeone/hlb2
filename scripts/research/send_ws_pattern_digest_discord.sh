...
MODE="${1:-scheduled}"
DIGEST_FILE="${2:-$ROOT_DIR/logs/ops/ws_pattern_digest_${MODE}.txt}"
WEBHOOK_URL="${WS_PATTERN_DISCORD_WEBHOOK_URL:-${DISCORD_WEBHOOK_URL:-}}"
...
if [ -z "$WEBHOOK_URL" ] && [ "$DRY_RUN" != "1" ]; then
  echo "[INFO] webhook is not set (WS_PATTERN_DISCORD_WEBHOOK_URL or DISCORD_WEBHOOK_URL), skip notify"
  exit 0
fi

NOW_UTC="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
TITLE="HLB2 WS Pattern ${MODE^^} Report"
...
echo "[OK] discord pattern digest sent"
